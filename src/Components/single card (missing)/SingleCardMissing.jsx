...
export default function SingleCardMissing() {
    ...
    async function getSingleCard() {
        try {
            const { data } = await axios.get(`${baseUrl}/missingPersons/getAllMissingPersons/${id}`)
            setCard(data.result)
        } catch {
                                    ...
                                    <div className={`${cardcss.personcard} mb-2 rounded-3 py-4 px-3`}>
                                        ...
                                        <div className={`w-100 d-flex`}>
                                            <div className='col-4 me-2 d-flex flex-column text-center'>
                                                <img className={`${cardcss.missingPhotos} mb-2 w-100 rounded-2`} src={card.images[0].secure_url} alt="missing person" />
