...
import listCss from './ListFound.module.css'
...
export default function List() {
    ...
    async function getAllData(currentPage) {
        const { data } = await axios.get(`${baseUrl}/missingPersons/getAllFoundPersons?page=${currentPage}`)
    ...
    async function searchByName(keyword) {
        try {
            const { data } = await axios.get(`${baseUrl}/missingPersons/foundPersons/all?keyword=${keyword}&page=${currentPage}`);
    ...
    async function searchMissingPersonsByArea(area) {
        try {
            const { data } = await axios.get(`${baseUrl}/missingPersons/getAllFoundPersonsWithArea?country=${area}&page=${currentPage}`);
            setSearchResults(data.results);
        } catch (error) {
    ...
    async function searchMissingPersonsByYear(year) {
        try {
            const { data } = await axios.get(`${baseUrl}/missingPersons/getAllFoundPersonsWithYear?year=${year}&page=${currentPage}`);
            setSearchResults(data.results);
        } catch (error) {
            ...
            <div className="container-fluid">
                <div className="row">
                    <div className="col-12 justify-content-center d-flex mb-4">
                        <div className={`col-4 w-25 position-relative me-5 text-center ${listCss.search}`}>
                    ...
                    <div className="d-flex flex-wrap col-12">
                        {searchResults && searchResults.length > 0 ? (
                            searchResults.map((search, index) => (<div key={index} className="size col-sm-12 col-md-6 col-lg-3">
                            ...
                            return <div key={idx} className="size col-sm-12 col-md-6 col-lg-3">
                                <div className={`${listCss.personcard} me-2 mb-2 rounded-3 py-4 px-3 overflow-hidden`}>
                                    ...
                                    {/* <div className='d-flex justify-content-center'>
                                        <Link to={`/FoundPersoninformation/${card._id}`} className={`${listCss.cursor} btn btn-outline-dark`}>Contact</Link>
                                    </div> */}
                                </div>
                            </div>
                        }))}
